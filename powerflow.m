...
for m=1:9
    for n=1:2
       ymatrix(branchdata.data(m,n),branchdata.data(m,n))=...
                      ymatrix(branchdata.data(m,n),branchdata.data(m,n))...
                      +1/(branchdata.data(m,3)+1i*branchdata.data(m,4))...
                      +1i*branchdata.data(m,5); 
    end
    ymatrix(branchdata.data(m,1),branchdata.data(m,2))=...
                         -1/(branchdata.data(m,3)+1i*branchdata.data(m,4));
    ymatrix(branchdata.data(m,2),branchdata.data(m,1))=...
                        ymatrix(branchdata.data(m,1),branchdata.data(m,2));
end
    ...
    for m=1:8
        for n=1:9
            if nodedata.data(n,1)== m
               deltap(m,1)=nodedata.data(n,2) -(nodedata.data(n,4))^2*...
                   (real(ymatrix1(nodedata.data(n,1),nodedata.data(n,1))));
               if m<7
                  deltaq(m,1)=nodedata.data(n,3)+(nodedata.data(n,4))^2*...
        ...
        for p=1:9
            if (branchdata.data(p,1)==m)
                 ...
                 deltap(m,1)= deltap(m,1)-nodedata.data(n,4)...
                                                    *nodedata.data(q,4)*...
              (real(ymatrix1(branchdata.data(p,1),branchdata.data(p,2)))...
                            *cos(nodedata.data(n,5)-nodedata.data(q,5))+...
               imag(ymatrix1(branchdata.data(p,1),branchdata.data(p,2)))...
                              *sin(nodedata.data(n,5)-nodedata.data(q,5)));
                 if m<7
                    deltaq(m,1)=deltaq(m,1)-nodedata.data(n,4)...
                                                    *nodedata.data(q,4)*...
              (real(ymatrix1(branchdata.data(p,1),branchdata.data(p,2)))...
                           *sin(nodedata.data(n,5)-nodedata.data(q,5))-...
               imag(ymatrix1(branchdata.data(p,1),branchdata.data(p,2)))...
...
for m=7:8
    ...
    for p=1:9
            if (branchdata.data(p,1)==m)
                 ...
                 PVQ((m-6),1)=PVQ((m-6),1)+nodedata.data(n,4)*...
                     nodedata.data(q,4)*...
              (real(ymatrix1(branchdata.data(p,1),branchdata.data(p,2)))...
                            *sin(nodedata.data(n,5)-nodedata.data(q,5))-...
               imag(ymatrix1(branchdata.data(p,1),branchdata.data(p,2)))...
...
for m=9:9
        ...
        for p=1:9
            if (branchdata.data(p,1)==m)
                ...
                balancePQ((m-8),1)=balancePQ((m-8),1)+nodedata.data(n,4)...
                                                    *nodedata.data(q,4)*...
              (real(ymatrix1(branchdata.data(p,1),branchdata.data(p,2)))...
                            *cos(nodedata.data(n,5)-nodedata.data(q,5))+...
               imag(ymatrix1(branchdata.data(p,1),branchdata.data(p,2)))...
                              *sin(nodedata.data(n,5)-nodedata.data(q,5)));
                balancePQ((m-7),1)=balancePQ((m-7),1)+nodedata.data(n,4)...
                                                    *nodedata.data(q,4)*...
              (real(ymatrix1(branchdata.data(p,1),branchdata.data(p,2)))...
                            *sin(nodedata.data(n,5)-nodedata.data(q,5))-...
               imag(ymatrix1(branchdata.data(p,1),branchdata.data(p,2)))...
...
for m=1:6
    if m<4
  ymatrix2(geneload(1,m),geneload(1,m)) =...
  ymatrix2(geneload(1,m),geneload(1,m))+ 1/(1i*nodesheet(geneload(1,m),6));    
    else
  ymatrix2(geneload(1,m),geneload(1,m))=...
      ymatrix2(geneload(1,m),geneload(1,m))+(-nodesheet(geneload(1,m),2)...
            +1i*nodesheet(geneload(1,m),3))/(nodesheet(geneload(1,m),4)^2);     
    end   
end
    ...
    for n=1:9       
        for p=1:9
            if (branchdata.data(p,1)==m)&&(branchdata.data(p,2)==n)
                Idis(m,n)=(Uf(1,m)-Uf(1,n))/(branchdata.data(p,3)+...
                    1i*branchdata.data(p,4));
            end    
